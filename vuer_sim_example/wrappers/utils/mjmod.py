...
import vuer_sim_example.wrappers.utils.transform_utils as trans
...
class CameraModder(BaseModder):
    ...
    def __init__(
        ...
        if camera_names is None:
            raise NotImplementedError("please provide the camera names, default behavior should be to randomize all of them though.")
        ...
        self.cam_name2id = {self.physics.model.camera(i).name: i for i in range(self.physics.model.ncam)}
...
class TextureModder(BaseModder):
    ...
    def __init__(
        ...
        random_state=None,
        geom_names=None,
        ignore_geom_names=None,
        randomize_local=False,
        randomize_local_geom_prefixes=None,
        ...
        def geom_ids_from_body(body_name, patterns):
            geoms = []
            for pattern in patterns:
                compiled_pattern = re.compile(pattern)
                if compiled_pattern.match(body_name):
                    # add all of the geoms in this body to the geom_names
                    for geomid in range(
                        self.model.body(body_name).geomadr[0], self.model.body(body_name).geomadr[0] + self.model.body(body_name).geomnum[0]
                    ):
                        geoms.append(geomid)
                        # geoms.append(self.model.geom(geomid).name)
                    break
            return geoms

        all_geom_ids = [i for i in range(self.model.ngeom)]
        model = self.model
        if geom_names is None:
            self.geom_ids = all_geom_ids
        else:
            self.geom_ids = []
            for bodyid in range(model.nbody):
                body_name = model.body(bodyid).name
                self.geom_ids.extend(geom_ids_from_body(body_name, geom_names))

            for geom_id in all_geom_ids:
                for pattern in geom_names:
                    compiled_pattern = re.compile(pattern)
                    if compiled_pattern.match(self.model.geom(geom_id)):
                        self.geom_ids.append(geom_id)
                        break

        if ignore_geom_names is not None:
            # need to remove the ignored geoms from the geom_names
            for bodyid in range(model.nbody):
                body_name = model.body(bodyid).name
                ignore_ids = geom_ids_from_body(body_name, ignore_geom_names)
                self.geom_ids = [id for id in self.geom_ids if id not in ignore_ids]

            for geom_id in all_geom_ids:
                for pattern in geom_names:
                    compiled_pattern = re.compile(pattern)
                    if compiled_pattern.match(self.model.geom(geom_id)):
                        self.geom_ids.remove(geom_id)
                        break

        print(f"Randomizing over {self.geom_ids} geoms")

        self.skybox_id = None
        for i in range(self.model.ngeom):
            if self.model.geom(i).name == "skybox":
                self.skybox_id = i
                break

        self.randomize_local_geom_ids = []
        if randomize_local_geom_prefixes is not None:
            for geom_id in self.geom_ids:
                geom_name = self.model.geom(geom_id).name
                if any([geom_name.startswith(prefix) for prefix in randomize_local_geom_prefixes]):
                    self.randomize_local_geom_ids.append(geom_id)

        self.randomize_local = randomize_local
        self.randomize_local_geom_prefixes = randomize_local_geom_prefixes or []
    ...
    def save_defaults(self):
        ...
        self._defaults = {k: {} for k in self.geom_ids}
        if self.randomize_skybox:
            self._defaults[self.skybox_id] = {}
        for id in self.geom_ids:
            if self._check_geom_for_texture(id):
                # store the texture bitmap for this geom
                tex_id = self._geom_to_tex_id(id)
                self._defaults[id]["texture"] = self._default_texture_bitmaps[tex_id]
                # store material properties as well (in tuple (reflectance, shininess, specular) form)
                self._defaults[id]["material"] = self.get_material(id)
            else:
                # store geom color
                self._defaults[id]["rgb"] = np.array(self.get_geom_rgb(id))

        if self.randomize_skybox:
            tex_id = self._geom_to_tex_id(self.skybox_id)
            self._defaults[self.skybox_id]["texture"] = self._default_texture_bitmaps[tex_id]

    def restore_defaults(self, geom_ids=None, skybox=False):
        """
        Reloads the saved parameter values.
        """
        if geom_ids is None:
        ...
        for id in geom_ids:
            if self._check_geom_for_texture(id):
                self.set_texture(id, self._defaults[id]["texture"], perturb=False)
                self.set_material(id, self._defaults[id]["material"], perturb=False)
            else:
                self.set_geom_rgb(id, self._defaults[id]["rgb"])
    ...
    def randomize(self):
        ...
        for id in self.geom_ids:
            if self._check_geom_for_texture(id):
                ...
                self._randomize_texture(id)
                # randomize material if requested
                if self.randomize_material:
                    self._randomize_material(id)
            else:
                # randomize geom color
                self._randomize_geom_color(id)

        if self.randomize_skybox:
            self._randomize_texture(self.skybox_id)

    def _randomize_geom_color(self, id):
        """
        ...
        """
        if self.randomize_local or id in self.randomize_local_geom_ids:
            random_color = self.random_state.uniform(0, 1, size=3)
            rgb = (1.0 - self.local_rgb_interpolation) * self._defaults[id]["rgb"] + self.local_rgb_interpolation * random_color
        else:
            rgb = self.random_state.uniform(0, 1, size=3)
        self.set_geom_rgb(id, rgb)

    def _randomize_texture(self, id):
        """
        ...
        self._texture_variation_callbacks[choice](id)

    def _randomize_material(self, id):
        """
    ...
    def rand_checker(self, id):
        ...
        self.set_checker(id, rgb1, rgb2, perturb=self.randomize_local)

    def rand_gradient(self, id):
        """
        ...
        self.set_gradient(id, rgb1, rgb2, vertical=vertical, perturb=self.randomize_local)

    def rand_rgb(self, id):
        """
        ...
        self.set_rgb(id, rgb, perturb=self.randomize_local)

    def rand_noise(self, id):
        """
        ...
        rgb1, rgb2 = self.get_rand_rgb(2)
        self.set_noise(id, rgb1, rgb2, fraction, perturb=self.randomize_local)
    ...
    def whiten_materials(self):
        ...
        for id in self.geom_ids:
            # whiten geom
            self.model.geom_rgba[id, :] = 1.0

            if self._check_geom_for_texture(id):
                # whiten material
                mat_id = self.model.geom_matid[id]
                self.model.mat_rgba[mat_id, :] = 1.0

    def get_geom_rgb(self, id):
        """
        ...
        """
        return self.model.geom_rgba[id, :3]

    def set_geom_rgb(self, id, rgb):
        """
    ...
    def set_texture(self, id, bitmap, perturb=False):
        """
        ...
        """
        bitmap_to_set = self.get_texture(id).bitmap
        if perturb:
            bitmap = (1.0 - self.local_rgb_interpolation) * self._defaults[id]["texture"] + self.local_rgb_interpolation * bitmap
    ...
    def set_material(self, id, material, perturb=False):
        """
        ...
        mat_id = self._geom_to_mat_id(id)
        if perturb:
            material = (1.0 - self.local_material_interpolation) * self._defaults[id][
                "material"
    ...
    def get_checker_matrices(self, id):
        """
    ...
    def set_checker(self, id, rgb1, rgb2, perturb=False):
        """
        ...
        """
        cbd1, cbd2 = self.get_checker_matrices(id)
        rgb1 = np.asarray(rgb1).reshape([1, 1, -1])
        rgb2 = np.asarray(rgb2).reshape([1, 1, -1])
        bitmap = rgb1 * cbd1 + rgb2 * cbd2

        self.set_texture(id, bitmap, perturb=perturb)

    def set_gradient(self, id, rgb1, rgb2, vertical=True, perturb=False):
        """
        ...
        bitmap = self.get_texture(id).bitmap
        h, w = bitmap.shape[:2]
        if vertical:
        ...
        self.set_texture(id, new_bitmap, perturb=perturb)

    def set_rgb(self, id, rgb, perturb=False):
        """
    ...
    def set_noise(self, id, rgb1, rgb2, fraction=0.9, perturb=False):
        """
    ...
    def _check_geom_for_texture(self, id):
        """
        ...
        if mat_id < 0:
            return False
        tex_id = self.model.mat_texid[mat_id][1]
        if tex_id < 0:
            return False
        return True

    def _geom_to_tex_id(self, id):
        """
        ...
        assert self._check_geom_for_texture(id)
        mat_id = self.model.geom_matid[id]
        tex_id = self.model.mat_texid[mat_id][1]
        return tex_id

    def _geom_to_mat_id(self, id):
        """
