from vuer_sim_example.wrappers.camera_wrapper import CameraWrapper, DepthWrapper, RGBDWrapper
...
def create_multiview_env(env, **camera_configs) -> CameraWrapper:
    ...
    for key, config in camera_configs.items():
        if config.get("depth", None):
            ...
            from vuer_sim_example.wrappers.lucid_wrapper import (
                SegmentationWrapper,
                OverlayWrapper,
                MidasDepthWrapper,
                MaskWrapper,
            ...
            from vuer_sim_example.wrappers.domain_randomization_wrapper import DomainRandomizationWrapper
            env = DomainRandomizationWrapper(env, image_key=key, **config)
        elif config.get("gsplat", None):
            from vuer_sim_example.wrappers.lucid_wrapper import (
                OverlayWrapper,
            )
            from vuer_sim_example.neverwhere.gsplat_wrapper import SplatRGBWrapper
            env = OverlayWrapper(env, image_key=key, **config)
