...
class BasePolicy:
    ...
    def __init__(self, config, model_path, rl_rate=50, policy_action_scale=0.25, qpos=None):
        """Initialize the base policy with configuration and model."""
        self.config = config
        # Initialize robot config
        self._init_robot_config(qpos)
    ...
    def _init_robot_config(self, qpos):
        """Initialize robot configuration and parameters."""
        self.robot = Robot(self.config)
        self.num_dofs = self.robot.NUM_JOINTS
        self.default_dof_angles = np.array(qpos) if qpos else np.array(self.robot.DEFAULT_DOF_ANGLES)
...
if __name__ == "__main__":
    ...
    policy = BasePolicy(config, model_path, qpos=args.qpos)
    policy.run()